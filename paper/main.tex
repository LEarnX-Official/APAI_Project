...
\item \textbf{Idea development and methodology.} \emph{TODO}
\item \textbf{Literature review.} \emph{TODO}
\item \textbf{Coding.} \emph{TODO}
\item \textbf{Experiments and analysis.} \emph{TODO}
\item \textbf{Writing.} \emph{TODO}
\end{itemize}
...
\vspace{0.3cm}
\noindent
For reference when completing the above, the work comprised: formulation of FER
as a few-shot problem and design of the held-out-emotion protocol; a survey of
FER2013 results and the few-shot learning literature; the data pipeline and
episodic sampler (\texttt{data.py}), architectures (\texttt{models.py}), MAML
inner and outer loops (\texttt{maml.py}), the training entry points
(\texttt{train\_maml.py}, \texttt{train\_baseline.py},
\texttt{train\_pretrained.py}) and evaluation code (\texttt{evaluate.py}); six
training runs together with the checkpoint-selection ablation; and this report
including the system diagram.
